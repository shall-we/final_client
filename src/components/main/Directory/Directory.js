...
const createFolderModalData = ["oneInputModal", 'folder-plus', '폴더 생성', '생성할 폴더명을 입력해주세요.', '생성'];
const shareFolderModalData = ["selectModal", 'folder-plus', '공유 폴더 초대', '해당 폴더로 초대할 직원을 선택해주세요.', '완료'];
const deleteFolderModalData = ["noticeModal", 'trash-alt', '폴더 삭제', '폴더를 정말 삭제하시겠습니까?', '삭제'];
...
class Directory extends React.Component {
    ...
    async handleChatDrawerOpen(user_id, friend_id) {
        ...
        //console.log('before', ChatActions);
        ...
        if (result === "success") {
            ...
            //onsole.log('notExits', ChatActions);
        ...
        <div className='context-menu' key={id}>
            <Link to={'/note/'+item.content}>
                <div className="file-list"
                    ...
                    {/* <Statebutton/> */}
                    {  (item.status === 'DELETED') ? (
                            <div className="recovery-icon" style={{display:'flex', flexDirection : 'row', justifyContent : "flex-end"}}>
                        ...
                        <div className="stateButton">
                            <div className="menu menu--button">
                                ...
                                {(item.status === "ACTIVED")?
                                (<div className="menu__item menu__item--share" onClick={(e)=>this.handleSetModal(modalList[15],this.props.publishNote,item.id,'http://192.168.0.103:3000/note/public/'+item.content)}>
                                    <i className="fa fa-share-alt-square menu__item-icon"/>
                                    <span className="menu__item-text">UnShare</span>
                                </div>
                                ):(
                                <div className="menu__item menu__item--share" onClick={(e)=>this.handleSetModal(modalList[7],this.props.activedNote,item.id,'http://192.168.0.103:3000/note/public/'+item.content)}>
