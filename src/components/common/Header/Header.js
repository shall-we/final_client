...
const Header = ({name,profile,logout, chatNoticeCount,handleEnterNotice}) => {
...

      {
          ...
          (name)?( [
            <div key='login' className={cx('login')}> <img src={profile} alt='이미지'/>
              ...
              <Link to="/note" onClick={handleEnterNotice}><div key='alert' className={cx('notify') + " " + cx('notification') + " " + cx('show-count')} dataCount = {chatNoticeCount}/></Link>
